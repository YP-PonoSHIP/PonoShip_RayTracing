...
%#codegen
    properties (Access = protected, Nontunable)
        ...
        pOutputCombinedPattern (1, 1) logical = false
        % whether to initialize output as complex or real
        pIsOutputComplex (1, 1) logical = true;        
    end
    ...
    methods (Abstract, Hidden)
        etype = getElementType(obj)
        %getElementType Returns the type of the element
    end
    ...
    methods (Access = {?phased.internal.AbstractElement,...
        ...
        function shouldRatateZ = shouldRotateZToAlign(obj) %#ok<MANU> 
            shouldRatateZ = false;
        end
    end
    ...
    methods (Access = protected)
        ...
        function validateInputsImpl(~,freq,angle)
            ...
            if cond
                coder.internal.errorIf(cond, ...
                     'shared_channel:arrayelemdef:InvalidFrequency');
            end
            ...
            if cond
                coder.internal.errorIf(cond, ...
                     'shared_channel:arrayelemdef:NeedTwoRows');
            end

            cond =  ~isreal(angle);
            if cond
                coder.internal.errorIf(cond, ...
                     'shared_channel:arrayelemdef:InvalidAngle');
            end
    ...
    methods (Access = protected)
        ...
        function resp = stepImpl(obj,freq,angArg, varargin)
            ...
            phased.internal.AbstractVarSizeEngine.validateSize(freq,angArg);
        ...
        function out = getOutputSizeImpl(obj)
            ...
            sz_freq = propagatedInputSize(obj,1);
            sz_ang = propagatedInputSize(obj,2);
            out = [sz_ang(2) sz_freq(2)];
    ...
    methods 
        ...
        %   See also phased, phased.ArrayResponse.
            ...
            phased.internal.AbstractVarSizeEngine.validateSize(freq,ang);
        ...
        function [BW,BWAng] = beamwidth(obj,freq,varargin)
        %beamwidth Compute the beamwidth for an element
        %   beamwidth(H,FREQ) plots the 2D power pattern (in dB) of element
        %   specified by H for all azimuth angles at a fixed elevation
        %   angle of 0 degrees. The plot display the half-power beamwidth
        %   (in degrees) at the frequency specified in FREQ (in Hz) and
        %   also the angles (in degrees) in azimuth at which the magnitude
        %   of the power pattern decreases by 3 dB from the peak of the
        %   main beam.
        %
        %   beamwidth(H,FREQ,Name,Value) computes and plots the beamwidth
        ...
        %   are
        %
        %                    Cut: The plane across which beamwidth should
        %                         be computed. It should be specified as a
        %                         string, 'Azimuth' for azimuth plane and
        %                         'Elevation' for elevation plane. The
        %                         default value is 'Azimuth'.
        %               CutAngle: A scalar which specifies the
        %                         corresponding angle (in degrees) for the
        %                         plane to get the required 2D cut. If the
        %                         Cut is 'Azimuth', then the CutAngle
        %                         (Elevation) should lie between -90 and 90
        %                         degrees, whereas if the Cut is
        %                         specified as 'Elevation' then the
        %                         CutAngle (Azimuth) should lie between
        %                         -180 and 180 degrees. The default
        %                         CutAngle value is 0 degrees.
        %                 dBDown: A positive scalar which specifies the
        %                         power value (in dB) from the peak of the
        %                         main lobe. The default value is 3dB,
        %                         which translates to Half-Power
        %                         Beamwidth. For calculation of First-Null
        %                         Beamwidth specify the dBDown value as
        %                         Inf.
        %
        %   [BW,Angles] = beamwidth(...) returns the angular beamwidth BW
        %   (in degrees). Angles is a 1-by-2 vector which gives the
        %   corresponding Angle values (in degrees) of the beamwidth 
        %   reported as BW.
        %
        %   % Example:
        %   %   Calculate the Half-Power Beamwidth of a Cosine Antenna
        %   %   Element with the CosinePower exponents set to 10 along the
        %   %   azimuth plane and at 0 degrees elevation. Assume the
        %   %   antenna operating frequency as 1GHz.
        %
        %   myAnt = phased.CosineAntennaElement('CosinePower',[10 10]);
        %   [BW,Ang] = beamwidth(myAnt,1e9);
        %
        %   See also phased, phased.CosineAntennaElement/pattern.

            narginchk(2,inf)
            nargoutchk(0,2)
            if ~coder.target('MATLAB') && ~nargout
                coder.internal.assert(false, ...
                    'shared_channel:arrayelemdef:CodegenNotSupported',...
                    'beamwidth');
            end
            sigdatatypes.validateFrequency(freq,'beamwidth','FREQ',...
                {'scalar','real'});
            if coder.target('MATLAB')
                p = inputParser;
                addParameter(p,'Cut','Azimuth');
                addParameter(p,'CutAngle',0);
                addParameter(p,'dBDown',3);

                parse(p,varargin{:});

                plane = p.Results.Cut;
                cutangle = p.Results.CutAngle;
                dBDown = p.Results.dBDown;

            else
                parms = struct('Cut',string(0),'CutAngle',uint32(0),...
                    'dBDown',uint32(0),'PropagationSpeed',uint32(0),...
                    'Weights',uint32(0));

                pstruct = eml_parse_parameter_inputs(parms,[],varargin{:});
                plane = eml_get_parameter_value(pstruct.Cut,'Azimuth',...
                    varargin{:});
                cutangle = eml_get_parameter_value(pstruct.CutAngle,0,...
                    varargin{:});
                dBDown = eml_get_parameter_value(pstruct.dBDown,3,...
                    varargin{:});
            end
            plane = validatestring(plane,{'Azimuth','Elevation'},...
                'beamwidth','Cut');
            ang = -180:0.01:180;
            if strcmp(plane,'Azimuth')
                isazplane = true;
                sigdatatypes.validateAngle(cutangle,'beamwidth',...
                    'CutAngle',{'<=',90,'>=',-90,'scalar'});
                azel = [ang;cutangle*ones(size(ang))];
            else
                isazplane = false;
                sigdatatypes.validateAngle(cutangle,'beamwidth',...
                    'CutAngle',{'<=',180,'>=',-180,'scalar'});
                [elang,azang] = phased.internal.flipElevationAngleGrid(ang,cutangle);
                azel = [azang(1:length(ang));elang(1:length(ang))];
            end
            validateattributes(dBDown,{'numeric'},...
                {'nonempty','real','scalar','positive','nonnan'},...
                'beamwidth','dBDown');
            ...
            clobj = clonecg(obj);
            resp_temp = step(clobj,freq,azel);
            if polflag
                resp = hypot(resp_temp.H,resp_temp.V);
            else
                resp = resp_temp;
            end
            powerPattern = mag2db(abs(resp));
            [BW,BWAng] = phased.internal.beamwidth(powerPattern,ang,dBDown);
            if ~nargout
                phased.internal.plotBeamwidth(resp,freq,ang,BWAng,...
                    dBDown,isazplane,cutangle);
            end
        end
    end
    ...
    methods 
        ...
        %   See also phased, phased.ArrayResponse.
            ...
            if ~isempty(coder.target)
                coder.internal.assert(false, ...
                    'shared_channel:arrayelemdef:CodegenNotSupported','pattern');
            end

            narginchk(2,inf);
            nargoutchk(0,3);
            phased.internal.AbstractVarSizeEngine.validateSize(freq);
            validateattributes(obj,...
                {'phased.internal.AbstractElement'},...
                {'scalar'},'pattern','H');
            
            [fc,~,plotArgs] = phased.internal.radiationpattern.parsePatternInputs(...
        ...
        %   See also phased, phased.ArrayResponse.
            ...
            if ~isempty(coder.target)
                coder.internal.assert(false, ...
                    'shared_channel:arrayelemdef:CodegenNotSupported','patternAzimuth');
            end
            ...
            [fc,~,plotArgs] = phased.internal.radiationpattern.parsePatternAzimuthInputs(...
                false,obj,freq,varargin{:});
        ...
        %                          calculated. Elevation must be between 
        %                          -180 and 180. The default value is
        %                          -90:90.
        %
        ...
        %   See also phased, phased.ArrayResponse.
            ...
            if ~isempty(coder.target)
                coder.internal.assert(false, ...
                    'shared_channel:arrayelemdef:CodegenNotSupported','patternElevation');
            end
            ...
            [fc,~,plotArgs] = phased.internal.radiationpattern.parsePatternElevationInputs(...
                false,obj,freq,varargin{:});
    ...
    methods (Hidden)
        ...
        %   See also phased, phased.ArrayResponse.
            ...
            if ~isempty(coder.target)
                coder.internal.assert(false, ...
                                      'shared_channel:arrayelemdef:CodegenNotSupported','plotResponse');
            end
            ...
            [freq,~,plotArgs] = phased.internal.radiationpattern.parsePlotResponseInputs(...
                false,obj,freq,varargin{:});
    ...
    methods (Hidden)
        ...
        function flag = isSingleElement(obj) %#ok<MANU>
            flag = true;
        end
        function [useSubarraySteering,useCustomSubarraySteering] = hasSubarraySteering(obj) %#ok<MANU>
            useSubarraySteering = false;
            useCustomSubarraySteering = false;
        end
